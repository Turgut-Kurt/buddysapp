...
class LoginPage extends React.Component {
  ...
  handleOpenURLAndroid(url) {
    ...
    this.props.navigation.navigate('PassChange', { uid: parsedUrl[parsedUrl.length - 2], token: parsedUrl[parsedUrl.length - 1] })
  }
  _handleSubmit = async () => {
    ...
    LoginManager.logInWithPermissions(['public_profile', 'email']).then(
      function (result) {
        if (result.isCancelled) {
          ...
          const fields = 'id, email, picture.type(large), birthday, name, gender,first_name'
